...
```bash
pcpr-sam-test$ sam build --use-container
```
...
```bash
pcpr-sam-test$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
pcpr-sam-test$ sam local start-api
pcpr-sam-test$ curl http://localhost:3000/
```
...
```bash
pcpr-sam-test$ sam logs -n HelloWorldFunction --stack-name pcpr-sam-test --tail
```
...
```bash
pcpr-sam-test$ pip install -r tests/requirements.txt --user
# unit test
pcpr-sam-test$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
pcpr-sam-test$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name pcpr-sam-test
```
